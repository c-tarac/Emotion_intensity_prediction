...
embeddings = KeyedVectors.load_word2vec_format( "/content/emotion-intensity-prediction-/keras_regression/twitter_sgns_subset.txt.gz" , binary=False ) 
...
full_voc_data  = [ ( row["Tweet"] , float( row["Rating"] )  ) for row in csv.DictReader(open("/content/emotion-intensity-prediction-/keras_regression/vocab.csv"), delimiter='\t', quoting=csv.QUOTE_NONE) ]
...
## NOW LOAD TEST DATA ##
TSdata = [ ( row["Tweet"] , float( row["Rating"] )  ) for row in csv.DictReader(open("/content/emotion-intensity-prediction-/keras_regression/test/anger_CNN-LSTM_input.txt"), delimiter='\t', quoting=csv.QUOTE_NONE) ]
...
for i in EMOS.split():
	currentemo=str(i)
	TRdata = [ ( row["Tweet"] , float( row["Rating"] )  ) for row in csv.DictReader(open("/content/emotion-intensity-prediction-/keras_regression/train/"+currentemo+"_tr_dv.csv"), delimiter='\t', quoting=csv.QUOTE_NONE) ]
